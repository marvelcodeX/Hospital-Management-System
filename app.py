...
def add_patient():
    if request.method == 'POST':
        ...
        doctor_allotted = request.form['doctor_allotted']
...
def get_patient_info():
    if request.method == 'POST':
        ...
        patient_data = [row for row in read_csv(patients_file) if row['name'] == patient_name]
        if not patient_data:
...
def check_doctor_availability():
    if request.method == 'POST':
        ...
        doctor_data = [row for row in read_csv(doctors_file) if row['name'] == doctor_name]
        if not doctor_data:
...
def get_test_results():
    if request.method == 'POST':
        patient_name = request.form['patient_name']
        test_results_data = [row for row in read_csv(test_results_file) if row['name'] == patient_name]
...
def get_prescription():
    if request.method == 'POST':
        patient_name = request.form['patient_name']
        prescription_data = [row for row in read_csv(prescription_file) if row['name'] == patient_name]
